...
)
{
  ...
  $gerrit_sql_file = '/tmp/gerrit_firstopenid.sql.yaml'
...
# sudo pip install pyyaml
  $src_yamlerb = 'gerrit_config/gsql/gerrit_firstopenid.sql.yaml.erb'
  ...
  # run sql
  exec { 'run firstopenidadmin':
            path    => ['/bin', '/usr/bin'],
            command => "python ${lib_path}/gerrit_runsql.py ${debug_opts} ${run_sql_config_opt} --logfile /tmp/gerrit_firstopenidadmin.log --loggername firstopenidadmin",
