# == sysadmin_config:http_proxy_host
...
# test : p -e "if $::http_proxy_host  { notice("\$::http_proxy_host") } else { notice('no proxy') }"
...
Facter.add("http_proxy_host") do
 ...
 setcode do
   ...
   host      = (proxy_uri != nil )? URI(proxy_url).host : nil
   set_facter(host)
 end
end
